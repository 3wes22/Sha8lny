...
class JobService:
    ...
    def add_skills_to_job(job: Job, skill_names: List[str], is_required: bool = True) -> None:
        ...
        for skill_name in skill_names:
            ...
            # Match an existing skill case-insensitively; ``.first()`` tolerates
            # residual case-variant duplicates instead of raising
            # MultipleObjectsReturned. The slug is auto-generated by Skill.save().
            skill = Skill.objects.filter(name__iexact=skill_name).first()
            if skill is None:
                skill = Skill.objects.create(name=skill_name, category='technical')
